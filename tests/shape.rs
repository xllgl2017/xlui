...
impl TestShape {
    ...
    fn border_with(&mut self, _: &mut Ui, v: f32) {
        self.style.border.left_width = v;
        self.style.border.right_width = v;
        self.style.border.top_width = v;
        self.style.border.bottom_width = v;
    ...
    fn border_radius(&mut self, _: &mut Ui, v: u8) {
        self.style.radius = Radius::same(v);
    ...
    fn border_radius_f32(&mut self, _: &mut Ui, v: f32) {
        self.style.radius = Radius::same(v as u8)
...
impl App for TestShape {
    fn draw(&mut self, ui: &mut Ui) {
        ui.horizontal(|ui| {
            let rectangle = Rectangle::new(VisualStyle::same(self.style.clone()), 200.0, 150.0)
            ...
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    ui.label("边框:");
                    ui.add(SpinBox::new(self.style.border.left_width, 1.0, 0.0..20.0).id("sbw")
                        .contact("sb").contact("tsbw").contact("tsb")
                        .connect(Self::border_with));
                    ui.add(Slider::new(self.style.border.left_width).with_range(0.0..20.0).id("sb")
                        .contact("sbw").contact("tsbw").contact("tsb")
                        .connect(Self::border_with));
                });
                ui.horizontal(|ui| {
                    ui.label("圆角:");
                    ui.add(SpinBox::new(self.style.radius.left_bottom, 1, 0..50).id("sbr").contact("sr").connect(Self::border_radius));
                    ui.add(Slider::new(self.style.radius.left_bottom as f32).with_range(0.0..50.0).id("sr").contact("sbr").connect(Self::border_radius_f32));
                });
                ui.horizontal(|ui| {
                    ui.label("偏移:");
                    ui.label("x:");
        ...
        ui.horizontal(|ui| {
            ...
            let triangle = Triangle::new().with_pos(p0, p1, p2).with_style(VisualStyle::same(self.style.clone()));
            ui.add(triangle);
            ui.add_space(20.0);
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    ui.label("边框:");
                    ui.add(SpinBox::new(self.style.border.left_width, 1.0, 0.0..20.0).id("tsbw")
                        .contact("tsb").contact("sb").contact("sbw")
                        .connect(Self::border_with));
                    ui.add(Slider::new(self.style.border.left_width).with_range(0.0..20.0).id("tsb")
        ...
        ui.horizontal(|ui| {
            ...
            let mut circle = Circle::new(50.0).with_id("circle");
            circle.set_style(VisualStyle::same(self.style.clone()));
            ui.add(circle);
            ui.add_space(120.0);
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    ui.label("边框:");
                    ui.add(SpinBox::new(self.style.border.left_width, 1.0, 0.0..20.0).id("csbw")
                        .contact("csb").contact("tsb").contact("sb").contact("sbw")
                        .connect(Self::border_with));
                    ui.add(Slider::new(self.style.border.left_width).with_range(0.0..20.0).id("csb")
    ...
    fn update(&mut self, ui: &mut Ui) {
        ...
        rectangle.style_mut().inactive = self.style.clone();
        let circle: &mut Circle = ui.get_widget("circle").unwrap();
        circle.style_mut().inactive = self.style.clone();
